...
{
    public static Dictionary<string, Effect> efectosGuardados = new Dictionary<string, Effect>();
    public static Dictionary<string, string> efectosCreados = new Dictionary<string, string>();
    ...
    public Effect(Expresion name, Declaracion Action, List<(Token, Token)> Params, string codigo, Token Targets = null, Token Context = null)
    {
        this.name = name;
        this.Action = Action;
    ...
    public override bool Semantica()
    {
        string nombre = " ";
        try{
            if(name.type() == Tipo.Cadena)
            {
                if(!name.Semantica()) return false;
                nombre = name.Ejecutar().ToString();
                if(efectosGuardados.ContainsKey(nombre)) return false;
            }
            else return false;
        }
        catch
        ...
        try{
            if(!Action.Semantica()) return false;
        }
        catch{
            return false;
        }

        if(!efectosGuardados.ContainsKey(nombre))
        {
            efectosGuardados.Add(nombre, this);
            efectosCreados.Add(nombre, codigo);
        }
        else{
            efectosGuardados[nombre] = this;
            efectosCreados[nombre] = codigo;
        }
         return true;
    }
